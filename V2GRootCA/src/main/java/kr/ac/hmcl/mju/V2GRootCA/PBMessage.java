...
import kr.ac.hmcl.mju.V2GRootCA.PkiMsg.PkiMsgFormat;
import kr.ac.hmcl.mju.V2GRootCA.ReqToIssueCert.ReqToIssueCertFormat;
import kr.ac.hmcl.mju.V2GRootCA.ReqToIssueCrossCert.ReqToIssueCrossCertFormat;
import kr.ac.hmcl.mju.V2GRootCA.ResWithCert.CertInfo;
import kr.ac.hmcl.mju.V2GRootCA.ResWithCert.ResWithCertFormat;
...
public class PBMessage {
	...
	public ReqToIssueCrossCertFormat getReqToIssueCrossCert(PkiMsgFormat pkiMsg) {
		ReqToIssueCrossCertFormat reqToIssueCrossCert = null;
		try {
			reqToIssueCrossCert = ReqToIssueCrossCertFormat.parseFrom(pkiMsg.getPayload().toByteArray());
		} catch (InvalidProtocolBufferException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		return reqToIssueCrossCert;
	}
	
	public byte[] setReqToIssueCrossCert(byte[] subjectCsrFile, String subjectCertName) {
		ReqToIssueCrossCertFormat reqToIssueCrossCert = null;
		ByteString subjectCsrFileByteString;
		subjectCsrFileByteString = ByteString.copyFrom(subjectCsrFile);
		
		reqToIssueCrossCert = ReqToIssueCrossCertFormat.newBuilder().setSubjectCsrFile(subjectCsrFileByteString)
				.setSubjectCertName(subjectCertName)
				.build();
		byte[] sendData = reqToIssueCrossCert.toByteArray();
		return sendData;
	}
